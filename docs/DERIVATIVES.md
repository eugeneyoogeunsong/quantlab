...
| Method | Price | Error |
|---|---|---|
| Closed form | 5.151995 | reference |
...
on one, so the barrier the lattice *enforces* is displaced by up to half a level,
and changing the step count moves that displacement erratically.
...
freedom: choose a stretch parameter λ ≥ 1 so that exactly *m* up-moves land on
the barrier:
...
Measured: 5.428 against the true 5.152, a 5% overstatement.
...
Implied volatility inverts Black-Scholes using vega as the derivative. Deep in or
out of the money, vega collapses toward zero and the Newton step divides by
...
`NaN` rather than a plausible-looking wrong number.

The original implementation instead retried from several random starting guesses.
Bisection is deterministic and provably convergent, so we prefer it here; the
Brenner-Subrahmanyam approximation supplies the initial guess.

---

## What the tests actually check

Not "does this reproduce the number it produced last time": nothing is pinned to
a value this code generated. The references are external:
...
- **American call = European call** without dividends (Shreve): early exercise is
  worthless, so the two must agree exactly.
