...
**Design note.** The correlation check counts *all* cells, including zero
weights, not only held positions. Most strategies equal-weight their picks, so
among held cells the weight is constant and the correlation is undefined — an
earlier version silently degraded to "warn" on exactly the strategies it most
needed to test. Including the zeros reframes the question as "did held names
outperform unheld names on the same bar", which is the leakage signature.
...
Default is 10 bps one-way for large-cap equity.
...
The HTML report puts the **QA verdict above the equity curve**, deliberately. If
a strategy failed its checks, that should be the first thing a reader sees, not a
