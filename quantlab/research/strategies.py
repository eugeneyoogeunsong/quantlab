...
A Strategy maps prices -> target weights. It does NOT decide position size in
dollars, apply risk limits, or know about costs; those are Layer 4 and Layer 3.
Keeping the seam here means you can run the same signal through three different
risk configurations and attribute the difference honestly.

The contract
------------
`generate_weights(prices) -> DataFrame` (date x symbol), where row `t` holds the
weights you *want* on date t, decided using information available at the close
of t. The backtester applies them at the next open. Strategies must never shift
signals forward themselves -- doing it in two places is how you get an
...
out-of-sample data is not the same as a blog backtest, and the report should
say which is which.
"""
...
class Strategy(ABC):
    ...
    def min_history(self) -> int:
        """Rows needed before the first valid signal. Used to trim warm-up."""
        return int(max([v for v in self.params.values() if isinstance(v, (int, float))] or [252]))

    def generate_weights(self, prices: pd.DataFrame) -> pd.DataFrame:
        """Default: rank the raw signal and go long the top decile-ish slice."""
...
class CrossSectionalMomentum(Strategy):
    ...
    skipping the most recent month, and found the top-minus-bottom decile earned
    roughly 1%/month over 1965-1989. It has been replicated across countries and
    asset classes and remains one of the most-studied anomalies in finance --
    while also being famous for rare, violent crashes during sharp market
    rebounds (2009), when the short leg of beaten-down names rockets.
...
class TimeSeriesMomentum(Strategy):
    """Hold each asset only while its own 12-month return is positive.
    ...
    Analogy: cross-sectional momentum always picks the fastest horse in the race.
    Time-series momentum will decline to bet at all if every horse is walking.
    """
    ...
    def generate_weights(self, prices: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.vol_target_scale:
            # Inverse-vol tilt inside the trend sleeve so a quiet bond ETF and a
            # loud commodity ETF contribute comparable risk, not comparable dollars.
        ...
        # Deliberately NOT renormalised to 1.0 when few assets trend: staying
        # partly in cash during downtrends is the strategy, not a bug.
...
class LowVolatility(Strategy):
    ...
    risk-adjusted returns than CAPM predicts. The leading explanation is
    leverage constraints: investors who want more return but cannot borrow bid
    up high-beta stocks instead, depressing their forward returns. Related to
    Frazzini & Pedersen's betting-against-beta factor.

    Caveat worth stating plainly: the raw return is usually *lower* than the
    market. The claim is about return per unit of risk, and capturing it in
    absolute terms historically required leverage -- which reintroduces exactly
    ...
    def raw_signal(self, prices: pd.DataFrame) -> pd.DataFrame:
        return -F.realized_vol(prices, self.vol_window)  # negate: higher = better
...
class MeanReversion(Strategy):
    """Buy short-term losers. The mirror image of momentum, at a shorter horizon.
    ...
    def raw_signal(self, prices: pd.DataFrame) -> pd.DataFrame:
        ...
        return -z  # most oversold = most attractive

    def generate_weights(self, prices: pd.DataFrame) -> pd.DataFrame:
        sig = self.raw_signal(prices)
        eligible = sig > -self.entry_z  # z below entry_z, sign already flipped
...
class DualMomentum(Strategy):
    ...
    Two filters stacked: rank assets against each other (cross-sectional), then
    require the winner to also be beating cash on its own (time-series). If the
    best available asset is still falling, hold nothing.

    Popularised by Gary Antonacci. The construction is a sensible combination of
    two independently well-evidenced effects, but note the specific published
    parameterisation has a much thinner out-of-sample record than the underlying
    momentum literature it draws on.
    """
    ...
    def generate_weights(self, prices: pd.DataFrame) -> pd.DataFrame:
        ...
        sel = (ranks <= self.top_n) & (absolute > 0)  # both gates must pass
        counts = sel.sum(axis=1)
        return sel.astype(float).div(counts.replace(0, np.nan), axis=0).fillna(0.0)
...
class BuyAndHold(Strategy):
    ...
    QA Section C requires a benchmark comparison. This is the honest one: if a
